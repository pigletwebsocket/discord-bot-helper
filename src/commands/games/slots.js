...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      // Check cooldown
      const cooldownInfo = await cooldowns.checkCommandCooldown(interaction.user.id, 'slots');
      ...
      // Set cooldown
      await cooldowns.setCommandCooldown(user.id, 'slots');
      ...
      if (isWin) {
        ...
        await db.updateStats(user.id, 'slots', 'win', betAmount, winnings);
      } else {
        updatedUser = await db.removeCash(user.id, betAmount);
        await db.updateStats(user.id, 'slots', 'loss', betAmount, 0);
      }
      ...
      await interaction.editReply({ content: 'There was an error playing slots. Please try again later.' });
    }
  }
};
